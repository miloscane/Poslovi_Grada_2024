...
function addRow(json){
	...
	row.setAttribute("class","row");
		...
		var cena = document.createElement("DIV");
		cena.setAttribute("class","elem cena");
		cena.innerHTML = json ? brojSaRazmacima(rowJson.podizvodjacPrice) : " ";
		if(json){
			cena.setAttribute("data-price",rowJson.podizvodjacPrice)
		}
		row.appendChild(cena);
		...
		var ukupno = document.createElement("DIV");
		ukupno.setAttribute("class","elem ukupno");
		ukupno.innerHTML = json ? brojSaRazmacima(parseFloat(rowJson.kolicina)*parseFloat(rowJson.podizvodjacPrice)) : " ";
		row.appendChild(ukupno);
		...
		suggestionBox.setAttribute("class","suggestionBox");
			...
			for(var i=0;i<suggestionsToShow.length;i++){
				...
				row.setAttribute("onclick","inputSuggestion(this)");
					...
					var cena = document.createElement("DIV");
					cena.setAttribute("class","elem cena");
					cena.innerHTML = brojSaRazmacima(suggestionsToShow[i].podizvodjacPrice);
					row.appendChild(cena);
...
function inputSuggestion(elem){
	...
	if(priceItem){
		...
		//podizvodjac
		parent.getElementsByClassName("cena")[1].innerHTML = brojSaRazmacima(priceItem.podizvodjacPrice);
		parent.getElementsByClassName("cena")[1].dataset.price = priceItem.podizvodjacPrice;
		...
		//podizvodjac
		parent.getElementsByClassName("ukupno")[1].innerHTML = brojSaRazmacima(priceItem.podizvodjacPrice);
	}else{
...
function calculateTable(){
	...
	for(var i=0;i<rows.length;i++){
		...
		//podizvodjac
		if(!isNaN(parseFloat(rows[i].getElementsByClassName("cena")[1].dataset.price))){
			ukupanIznosPodizvodjaca = ukupanIznosPodizvodjaca + parseFloat(rows[i].getElementsByClassName("cena")[1].dataset.price)*rows[i].getElementsByClassName("kolicina")[0].getElementsByTagName("INPUT")[0].value;
		}
	}
	document.getElementById("tabela-iznos").innerHTML = brojSaRazmacima(ukupanIznos)+" дин.";
	document.getElementById("tabela-iznos-podizvodjac").innerHTML = brojSaRazmacima(ukupanIznosPodizvodjaca)+" дин.";
...
function quantityInput(elem){
	...
	//podizvodjac
	elem.parentElement.parentElement.getElementsByClassName("ukupno")[1].innerHTML = brojSaRazmacima(elem.value*parseFloat(elem.parentElement.parentElement.getElementsByClassName("cena")[1].dataset.price));
	calculateTable();
}

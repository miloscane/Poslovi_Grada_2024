...
var podizvodjaci  = ["SeHQZ--1672650353244","IIwY4--1672650358507","e3MHS--1675759749849","eupy8--1676039178890","S5mdP--1677669290493","0ztkS--1672041761145","ylSnq--1672041756318","mile--1672650353244","LLgsa--1672650358507"];
...
var istok         = ["ZVEZDARA","RAKOVICA","VOŽDOVAC","PALILULA","VRAČAR"];
var zapad         = ["NOVI BEOGRAD","ZEMUN","ČUKARICA","SAVSKI VENAC","STARI GRAD"];
...
//ertik, izmedju
    ...
    var json = response.data;
		...
		console.log("**********************  Cekirano Crpljenje i konstatacija *****************")
		for(var i=0;i<izvestaji.length;i++){
			for(var j=0;j<izvestaji[i].nalozi.length;j++){
				if(izvestaji[i].nalozi[j].tipoviRada.indexOf("Црпљење")>=0 && izvestaji[i].nalozi[j].tipoviRada.indexOf("Констатација")>=0){
					console.log(izvestaji[i].majstorInfo.ime + " "+reshuffleDate(izvestaji[i].date))
					console.log("----------------")
					break;
				}
		...
		console.log("**********************  Cekirano Popis i konstatacija *****************")
		for(var i=0;i<izvestaji.length;i++){
			for(var j=0;j<izvestaji[i].nalozi.length;j++){
				if(izvestaji[i].nalozi[j].tipoviRada.indexOf("Попис")>=0 && izvestaji[i].nalozi[j].tipoviRada.indexOf("Констатација")>=0){
					console.log(izvestaji[i].majstorInfo.ime + " "+reshuffleDate(izvestaji[i].date))
					console.log("----------------")
					break;
				}
		...
		/*var proizvodi = await proizvodiDB.find({}).toArray();
		var reversi = await magacinReversiDB.find({}).toArray();
		for(var i=0;i<proizvodi.length;i++){
			...
			for(var j=0;j<reversi.length;j++){
				for(var k=0;k<reversi[j].zaduzenje.length;k++){
					if(reversi[j].zaduzenje[k].uniqueId==proizvodi[i].uniqueId){
						...
						proizvodi[i].kolicina = proizvodi[i].kolicina + uzeto - vraceno;
					}
				}
			}
		}
		proizvodi.sort((a, b) => b.kolicina - a.kolicina);
		for(var i=0;i<proizvodi.length;i++){
			console.log(proizvodi[i].name + " - "+proizvodi[i].kolicina.toFixed(2) + " "+proizvodi[i].unit);
		}

		console.log("Done")*/

		/*var iznosNaloga = 0;
		var nalozi = await naloziDB.find({"prijemnica.datum.datum":{$regex:"11.2025"}}).toArray();
		for(var i=0;i<nalozi.length;i++){
			iznosNaloga = iznosNaloga + parseFloat(nalozi[i].ukupanIznos);

		}
		console.log(brojSaRazmacima(iznosNaloga))
		console.log(nalozi.length)*/
		...
		for(var i=0;i<poredjenje.length;i++){
			for(var j=0;j<nalozi.length;j++){
				if(nalozi[j].prijemnica){
					if(nalozi[j].prijemnica.datum.datum.includes(poredjenje[i].datum)){
						poredjenje[i].nalozi.push(nalozi[j])
					}
				}else{
					if(nalozi[j].datum.includes(poredjenje[i].datum)){
						poredjenje[i].nalozi.push(nalozi[j])
					}
				}
		...
		for(var i=0;i<poredjenje.length;i++){
			console.log(poredjenje[i].datum);
			var ukupanIznos = 0;
			for(var j=0;j<poredjenje[i].nalozi.length;j++){
				var iznosNaloga = isNaN(parseFloat(poredjenje[i].nalozi[j].ukupanIznos)) ? 0 : parseFloat(poredjenje[i].nalozi[j].ukupanIznos)
				ukupanIznos = ukupanIznos + iznosNaloga;
			}
			console.log("   Iznos: "+brojSaRazmacima(ukupanIznos))
			console.log("   Broj naloga: "+poredjenje[i].nalozi.length)
			console.log("   Prosek naloga: "+brojSaRazmacima(ukupanIznos/poredjenje[i].nalozi.length))
		}*/
		...
		for(var i=0;i<json.length;i++){
			for(var j=0;j<nalozi.length;j++){
				if(nalozi[j].prijemnica.datum.datum.includes(json[i].datum)){
					json[i].nalozi.push(nalozi[j])
				}
			}
		}

		for(var i=0;i<json.length;i++){
			for(var j=0;j<json[i].nalozi.length;j++){
				
				if(json[i].nalozi[j].opis){
					if(json[i].nalozi[j].opis.toLowerCase().includes("amena")){
						json[i].ukupanIznos = json[i].ukupanIznos + parseFloat(json[i].nalozi[j].ukupanIznos);
						if(parseFloat(json[i].nalozi[j].ukupanIznos)<50000){
							json[i].zameneDo50++;
						}else if(parseFloat(json[i].nalozi[j].ukupanIznos)<100000 && parseFloat(json[i].nalozi[j].ukupanIznos)>=50000){
							json[i].zameneDo100++;
						}else if(parseFloat(json[i].nalozi[j].ukupanIznos)<150000 && parseFloat(json[i].nalozi[j].ukupanIznos)>=100000){
							json[i].zameneDo150++;
						}else if(parseFloat(json[i].nalozi[j].ukupanIznos)>=150000){
							json[i].zamenePreko150++;
						}
					}
				}
				
			}
			json[i].nalozi = json[i].nalozi.length;
		...
		var woma	= ["80.02.09.003","80.02.09.004","80.02.09.020","80.02.09.021","80.02.09.022","80.02.09.023","80.02.09.024","80.02.09.025","80.02.09.026","80.02.09.027","80.02.09.028","80.02.09.029","80.04.01.006","80.02.09.031"];
		var crp   = ["80.02.09.007","80.02.09.008","80.02.09.009","80.02.09.010","80.02.09.011","80.02.09.012","80.02.09.013","80.02.09.014","80.02.09.015","80.02.09.016","80.02.09.017","80.02.09.018","80.04.01.012"];
		var kop 	= ["80.03.01.001","80.03.01.002","80.03.01.003","80.03.01.004","80.03.01.005","80.03.01.006","80.03.01.007","80.03.01.008","80.03.01.009","80.03.01.010","80.03.01.011","80.03.01.012","80.03.01.013","80.03.01.014","80.03.01.015","80.03.01.016","80.03.01.017","80.03.01.018","80.03.01.025","80.03.01.028","80.03.04.001","80.03.04.002","80.03.04.003","80.03.04.004","80.03.04.005","80.03.04.006","80.03.04.007","80.03.04.008","80.03.04.009","80.03.04.010","80.03.04.011","80.03.04.012","80.03.04.013","80.03.08.001","80.03.08.005","80.03.08.006","80.03.08.008","80.03.08.011"];
		var mas 	= ["80.03.01.019","80.03.01.020","80.03.01.021","80.03.01.022","80.03.01.023","80.03.01.024","80.03.01.026","80.03.01.027","80.03.01.029","80.03.01.030","80.03.04.014","80.03.04.015","80.03.04.023","80.03.04.024","80.03.04.028","80.03.04.030","80.03.04.032","80.03.04.037","80.03.04.038"];
		var odgusenja = ["80.02.09.001","80.02.09.002","80.02.09.005","80.02.09.006","80.02.10.003","80.02.10.004","80.02.10.005","80.02.10.007"];
		...
		for(var i=0;i<json.length;i++){
			//delete json[i].nalozi;
			for(var j=0;j<nalozi.length;j++){
				if(nalozi[j].prijemnica.datum.datum.includes(json[i].datum)){
					for(var k=0;k<nalozi[j].obracun.length;k++){
						var iznos = isNaN(parseFloat(nalozi[j].obracun[k].price)) ? 0 : parseFloat(nalozi[j].obracun[k].price);
						if(woma.indexOf(nalozi[j].obracun[k].code)>=0){
							
							json[i].woma = json[i].woma+iznos;
						}else if(crp.indexOf(nalozi[j].obracun[k].code)>=0){
							json[i].crpljenjeIDezinfekcija = json[i].crpljenjeIDezinfekcija+iznos;
						}else if(kop.indexOf(nalozi[j].obracun[k].code)>=0){
							json[i].rucnoKopanje = json[i].rucnoKopanje+iznos;
						}else if(mas.indexOf(nalozi[j].obracun[k].code)>=0){
							json[i].masinskoKopanje = json[i].masinskoKopanje+iznos;
						}else if(odgusenja.indexOf(nalozi[j].obracun[k].code)>=0){
							json[i].odgusenje = json[i].odgusenje+iznos;
						}else if(finalizacija.indexOf(nalozi[j].obracun[k].code)>=0){
							json[i].finalizacija = json[i].finalizacija+iznos;
						}else if(zamene.indexOf(nalozi[j].obracun[k].code)>=0){
							json[i].zamene = json[i].zamene+iznos;
							json[i].nalozi.push(nalozi[j]);
							break;
						}
					}
		...
		for(var i=0;i<json.length;i++){
			delete json[i].woma;
			delete json[i].crpljenjeIDezinfekcija;
			delete json[i].rucnoKopanje;
			delete json[i].masinskoKopanje;
			delete json[i].odgusenje;
			delete json[i].finalizacija;
			delete json[i].zamene;
			json[i].brojZamena = json[i].nalozi.length;
			json[i].zamene20_40 = 0;
			json[i].zamene40_60 = 0;
			json[i].zamene60_100 = 0;
			json[i].zamene100_140 = 0;
			json[i].zamene140_180 = 0;
			json[i].zamene180_250 = 0;
			json[i].zamenePreko250 = 0;
			for(var j=0;j<json[i].nalozi.length;j++){
				var iznos = isNaN(parseFloat(nalozi[j].ukupanIznos)) ? 0 : parseFloat(nalozi[j].ukupanIznos);
				if(iznos>=20000 && iznos<=40000){
					json[i].zamene20_40++;
				}else if(iznos>40000 && iznos<=60000){
					json[i].zamene40_60++;
				}else if(iznos>60000 && iznos<=100000){
					json[i].zamene60_100++;
				}else if(iznos>100000 && iznos<=140000){
					json[i].zamene100_140++;
				}else if(iznos>140000 && iznos<=180000){
					json[i].zamene140_180++;
				}else if(iznos>180000 && iznos<=250000){
					json[i].zamene180_250++;
				}else if(iznos>260000){
					json[i].zamenePreko250++;
				}
			}
			delete json[i].nalozi;
		}
		...
		/*var adrese = [];
		var nalozi2022 = await nalozi2022DB.find({}).toArray();
		var nalozi2023 = await nalozi2023DB.find({}).toArray();
		var nalozi2024 = await nalozi2024DB.find({}).toArray();
		var nalozi = await naloziDB.find({}).toArray();
		var elektroNalozi = await client.db("ProLineElektro").collection('Nalozi').find({}).toArray();

		for(var i=0;i<nalozi2024.length;i++){
			var adresaJson = {};
			adresaJson.adresa = nalozi2024[i].adresa;
			adresaJson.radnaJedinica = nalozi2024[i].radnaJedinica;
			adresaJson.nalozi = [];
			if(nalozi2024[i].coordinates){
				adresaJson.coordinates = nalozi2024[i].coordinates;
			}

			
			var exists = adrese.some(a =>
	        a.adresa === nalozi2024[i].adresa &&
	        a.radnaJedinica === nalozi2024[i].radnaJedinica
	    );
	    if(!exists){
	    	adrese.push(adresaJson);
	    }
		}

		for(var i=0;i<nalozi.length;i++){
			var adresaJson = {};
			adresaJson.adresa = nalozi[i].adresa;
			adresaJson.radnaJedinica = nalozi[i].radnaJedinica;
			adresaJson.nalozi = [];
			if(nalozi[i].coordinates){
				adresaJson.coordinates = nalozi[i].coordinates;
			}

			var exists = adrese.some(a =>
	        a.adresa === nalozi[i].adresa &&
	        a.radnaJedinica === nalozi[i].radnaJedinica
	    );
	    if(!exists){
	    	adrese.push(adresaJson);
	    }
		...
		for(var i=0;i<elektroNalozi.length;i++){
			var adresaJson = {};
			adresaJson.adresa = elektroNalozi[i].adresa;
			adresaJson.radnaJedinica = elektroNalozi[i].radnaJedinica;
			adresaJson.nalozi = [];
			if(elektroNalozi[i].coordinates){
				adresaJson.coordinates = elektroNalozi[i].coordinates;
			}

			var exists = adrese.some(a =>
	        a.adresa === elektroNalozi[i].adresa &&
	        a.radnaJedinica === elektroNalozi[i].radnaJedinica
	    );
	    if(!exists){
	    	adrese.push(adresaJson);
	    }
		...
		for(var i=0;i<nalozi2022.length;i++){
			var adresaJson = {};
			adresaJson.adresa = nalozi2022[i].adresa.split(",")[0];
			adresaJson.radnaJedinica = nalozi2022[i].radnaJedinica;
			adresaJson.nalozi = [];
			if(nalozi2022[i].coordinates){
				adresaJson.coordinates = nalozi2022[i].coordinates;
			}

			
			var exists = adrese.some(a =>
	        a.adresa === nalozi2022[i].adresa &&
	        a.radnaJedinica === nalozi2022[i].radnaJedinica
	    );
	    if(!exists){
	    	adrese.push(adresaJson);
	    }
		...
		for(var i=0;i<nalozi2023.length;i++){
			var adresaJson = {};
			adresaJson.adresa = nalozi2023[i].adresa.split(",")[0];
			adresaJson.radnaJedinica = nalozi2023[i].radnaJedinica;
			adresaJson.nalozi = [];
			if(nalozi2023[i].coordinates){
				adresaJson.coordinates = nalozi2023[i].coordinates;
			}

			
			var exists = adrese.some(a =>
	        a.adresa === nalozi2023[i].adresa &&
	        a.radnaJedinica === nalozi2023[i].radnaJedinica
	    );
	    if(!exists){
	    	adrese.push(adresaJson);
	    }
		...
		console.log("Adrese: "+adrese.length)
		console.log("Nalozi: "+eval(nalozi2022.length+nalozi2023.length+nalozi2024.length+nalozi.length+elektroNalozi.length));

		for(var i=0;i<adrese.length;i++){
			for(var j=0;j<nalozi2022.length;j++){
				if(nalozi2022[j].adresa.includes(adrese[i].adresa) || nalozi2022[j].adresa==adrese[i].adresa){
					adrese[i].nalozi.push(nalozi2022[j].broj)
				}
			}
			for(var j=0;j<nalozi2023.length;j++){
				if(nalozi2023[j].adresa.includes(adrese[i].adresa) || nalozi2023[j].adresa==adrese[i].adresa){
					adrese[i].nalozi.push(nalozi2023[j].broj)
				}
			}
			for(var j=0;j<nalozi2024.length;j++){
				if(nalozi2024[j].adresa.includes(adrese[i].adresa) || nalozi2024[j].adresa==adrese[i].adresa){
					adrese[i].nalozi.push(nalozi2024[j].broj)
				}
			}
			for(var j=0;j<nalozi.length;j++){
				if(nalozi[j].adresa.includes(adrese[i].adresa) || nalozi[j].adresa==adrese[i].adresa){
					adrese[i].nalozi.push(nalozi[j].broj)
				}
			}
			for(var j=0;j<elektroNalozi.length;j++){
				if(elektroNalozi[j].adresa.includes(adrese[i].adresa) || elektroNalozi[j].adresa==adrese[i].adresa){
					adrese[i].nalozi.push(elektroNalozi[j].broj)
				}
			}
		}
		...
		fs.writeFileSync("adrese.json",JSON.stringify(adrese),{encoding:"utf8"})
		console.log("Wrote file");*/


		/*var adrese = JSON.parse(fs.readFileSync("adreseSaKoordinatama.json",{encoding:"utf8"}));
		//console.log(adrese)
		var counter = 0;
		for (let i = 0; i < adrese.length; i++) {
		  //adrese[i].coordinates = null;
		  if(!adrese[i].coordinates){
		  	counter++;
		  	/*try {
			    const street = encodeURIComponent((adrese[i].adresa || "").trim());
			    const county = encodeURIComponent((adrese[i].radnaJedinica || "").trim());

			    const url =
			      "http://192.168.100.4:8080/search" +
			      "?format=json" +
			      "&limit=1" +
			      "&street=" + street +
			      "&county=" + county +
			    ...
			    const response = await axios.get(url, {
			      headers: {
			        "User-Agent": "MyApp/1.0 (miloscane@gmail.com)"
			      }
			    });
			    ...
			    if (response.data.length > 0) {
			      ...
			      adrese[i].coordinates = {
			        lat: parseFloat(result.lat),
			        lng: parseFloat(result.lon)
			      };
			      //fs.writeFileSync("adreseSaKoordinatama.json",JSON.stringify(adrese),{encoding:"utf8"})
			      console.log(i+"/"+adrese.length)
			    } else {
			      console.log("No results found for", adrese[i].adresa);
			    }

			    console.log("--------------------");
			    await sleep(50); // very good 👍 respecting rate limits
			  } catch (err) {
			    const status = err?.response?.status;
			    console.error(`Error for "${adrese[i].adresa}":`, status || err.message);
			  }
		  }
...
//		fs.writeFileSync("adreseSaKoordinatama.json",JSON.stringify(adrese),{encoding:"utf8"})
	//	console.log("Wrote file")
		/*var izvestajiZaPrint = [];
		var nalogBroj = "1856347";
		var majstori = await majstoriDB.find({}).toArray();
		var izvestaji = await dnevniIzvestajiDB.find({}).toArray();
		for(var i=0;i<izvestaji.length;i++){
			var nalogFound = false;
			for(var j=0;j<izvestaji[i].nalozi.length;j++){
				if(izvestaji[i].nalozi[j].broj==nalogBroj){
					izvestajiZaPrint.push(izvestaji[i]);
				}
			}
		}


		for(var i=0;i<izvestajiZaPrint.length;i++){

			for(var j=0;j<majstori.length;j++){
				if(majstori[j].uniqueId==izvestajiZaPrint[i].majstor){
					izvestajiZaPrint[i].majstor = majstori[j].ime;
				}
			}
		...
		/*const targets = [ 'spit', 'stan', 'st.'];
		const excludeTargets = ['dgu', 'agu'];
		//const targets = ['spit', 'stan', 'st.'];
		//const excludeTargets = ['ertik','dgu', 'agu','vert','horiz','hor'];

		function matches(text) {
			const value = text.toLowerCase();

			const hasTarget = targets.some(t => value.includes(t));
			const hasExclude = excludeTargets.some(e => value.includes(e));
		...
		for(var i=0;i<nalozi.length;i++){
			nalozi[i].podizvodjac = 0;
			if(podizvodjaci.indexOf(nalozi[i].majstor)>=0){
				nalozi[i].podizvodjac = 1;
			}

			var mesecFound = false;
			for(var j=0;j<meseci2025.length;j++){
				var mesec = meseci2025[j];
				if(nalozi[i].datum.datum.includes(mesec.mesecStr)){
					mesec.nalozi.push(nalozi[i]);
					mesecFound = true;
					break;
				}
			}
			if(!mesecFound){
				for(var j=0;j<meseci2024.length;j++){
					var mesec = meseci2024[j];
					if(nalozi[i].datum.datum.includes(mesec.mesecStr)){
						mesec.nalozi.push(nalozi[i]);
						break;
					}
		...
		for(var i=0;i<meseci2024.length;i++){
			var mesec = meseci2024[i];

			mesec.wome = {};
			mesec.wome.iznos = 0;

			mesec.crpljenje = {};
			mesec.crpljenje.iznos = 0;

			mesec.rucnoKopanje = {};
			mesec.rucnoKopanje.iznos = 0;

			mesec.bager = {};
			mesec.bager.iznos = 0;

			mesec.odgusenje = {};
			mesec.odgusenje.iznos = 0;

			mesec.finalizacija = {};
			mesec.finalizacija.iznos = 0;

			mesec.zamene = {};
			mesec.zamene.iznos = 0;

		}

		for(var i=0;i<meseci2025.length;i++){
			var mesec = meseci2025[i];
		...
		var cenovnik2024KategorijeString = fs.readFileSync("cenovnik2024Kategorije.csv",{encoding:"utf8"});
		var cenovnik2024KategorijeArray = cenovnik2024KategorijeString.split("\r\n");

		for(var i=0;i<cenovnik2024.length;i++){
			for(var j=0;j<cenovnik2024KategorijeArray.length;j++){
				if(cenovnik2024[i].code==cenovnik2024KategorijeArray[j].split(",")[0]){
					cenovnik2024[i].kategorija = cenovnik2024KategorijeArray[j].split(",")[1];
				}
			}
		}
		...
		for(var i=0;i<meseci2024.length;i++){
			var mesec = meseci2024[i];
			for(var j=0;j<mesec.nalozi.length;j++){
				var nalog = mesec.nalozi[j];
				for(var k=0;k<nalog.obracun.length;k++){
					var stavka = nalog.obracun[k];
					var nalogObradjen = false;
					if(nalog.digitalizacija){
						if(nalog.digitalizacija.stambeno){
							if(nalog.digitalizacija.stambeno=="VIK 2025"){
								nalogObradjen = true;
								var price = 0;
								for(var l=0;l<cenovnik.length;l++){
									if(cenovnik[l].code==stavka.code){
										price = parseFloat(cenovnik[l].price);
										break;
									}
								}
								if(price>0){
									if(woma.indexOf(stavka.code)>=0){
										mesec.wome.iznos = mesec.wome.iznos + parseFloat(stavka.quantity)*price;
									}else if(crp.indexOf(stavka.code)>=0){
										mesec.crpljenje.iznos = mesec.crpljenje.iznos + parseFloat(stavka.quantity)*price;
									}else if(kop.indexOf(stavka.code)>=0){
										mesec.rucnoKopanje.iznos = mesec.rucnoKopanje.iznos + parseFloat(stavka.quantity)*price;
									}else if(mas.indexOf(stavka.code)>=0){
										mesec.bager.iznos = mesec.bager.iznos + parseFloat(stavka.quantity)*price;
									}else if(odgusenja.indexOf(stavka.code)>=0){
										mesec.odgusenje.iznos = mesec.odgusenje.iznos + parseFloat(stavka.quantity)*price;
									}else if(finalizacija.indexOf(stavka.code)>=0){
										mesec.finalizacija.iznos = mesec.finalizacija.iznos + parseFloat(stavka.quantity)*price;
									}else if(zamene.indexOf(stavka.code)>=0){
										mesec.zamene.iznos = mesec.zamene.iznos + parseFloat(stavka.quantity)*price;
									}
								}
					...
					if(!nalogObradjen){
						var price = 0;
						var kategorija = "";
						for(var l=0;l<cenovnik2024.length;l++){
							if(cenovnik2024[l].code==stavka.code){
								price = parseFloat(cenovnik2024[l].price)
								kategorija = cenovnik2024[l].kategorija;
								break;
							}
						}
						if(price>0){
							if(kategorija=="WOMA"){
								mesec.wome.iznos = mesec.wome.iznos + parseFloat(stavka.quantity)*price;
							}else if(kategorija=="CRPLJENJE"){
								mesec.crpljenje.iznos = mesec.crpljenje.iznos + parseFloat(stavka.quantity)*price;
							}else if(kategorija=="KOPANJE"){
								mesec.rucnoKopanje.iznos = mesec.rucnoKopanje.iznos + parseFloat(stavka.quantity)*price;
							}else if(kategorija=="BAGER"){
								mesec.bager.iznos = mesec.bager.iznos + parseFloat(stavka.quantity)*price;
							}else if(kategorija=="SAJLA"){
								mesec.odgusenje.iznos = mesec.odgusenje.iznos + parseFloat(stavka.quantity)*price;
							}else if(kategorija=="FINALIZACIJA"){
								mesec.finalizacija.iznos = mesec.finalizacija.iznos + parseFloat(stavka.quantity)*price;
							}else if(kategorija=="ZAMENA"){
								mesec.zamene.iznos = mesec.zamene.iznos + parseFloat(stavka.quantity)*price;
							}
						}
					}
		...
		fs.writeFileSync("nalozi2024.json",JSON.stringify(meseci2024),{encoding:"utf8"})
		console.log("Gotova 2024")
		...
		fs.writeFileSync("nalozi2025.json",JSON.stringify(meseci2025),{encoding:"utf8"})
		console.log("Gotova 2025")*/
		...
		/*for(var i=0;i<meseci2025.length;i++){
			meseci2025[i].iznosi = [];
			meseci2025[i].iznosiPodizvodjaca = [];
		}

		for(var i=0;i<meseci2024.length;i++){
			meseci2024[i].iznosi = [];
			meseci2024[i].iznosiPodizvodjaca = [];
		}
		...
		for(var i=0;i<nalozi.length;i++){
			for(var j=0;j<meseci2025.length;j++){
				...
				if(nalozi[i].datum.datum.includes(mesec.mesecStr)){
					if(nalozi[i].opis){
						if(matches(nalozi[i].opis)){
							if(nalozi[i].statusNaloga=="Fakturisan"){
								var iznos = isNaN(parseFloat(nalozi[i].ukupanIznos)) ? 0 : parseFloat(nalozi[i].ukupanIznos);
								if(iznos>0){
									if(podizvodjaci.indexOf(nalozi[i].majstor)>=0){
										mesec.iznosiPodizvodjaca.push(iznos);
									}else{
										mesec.iznosi.push(iznos);
									}
								}
							}
						}
  ...
  "1872346"
		...
		var nalozi = await naloziDB.find({broj:{$in:acaNalozi}}).toArray();
		var reversi = await magacinReversiDB.find({nalog:{$in:acaNalozi}}).toArray();
		var proizvodi = await proizvodiDB.find({}).toArray();
		var naloziJson = []
		for(var i=0;i<nalozi.length;i++){
			nalozi[i].reversi = [];
				...
				if(reversi[j].nalog==nalozi[i].broj){
					for(var k=0;k<reversi[j].zaduzenje.length;k++){
						for(var l=0;l<proizvodi.length;l++){
							if(proizvodi[l].uniqueId==reversi[j].zaduzenje[k].uniqueId){
								reversi[j].zaduzenje[k].naziv = proizvodi[l].name;
								reversi[j].zaduzenje[k].unit = proizvodi[l].unit;
								reversi[j].zaduzenje[k].price = parseFloat(proizvodi[l].price);
							}
						}
					}
					nalozi[i].reversi.push(reversi[j])
				}
			}
		}

		for(var i=0;i<nalozi.length;i++){
			nalozi[i].ukupanIznosPodizvodjaca = 0;
			for(var j=0;j<nalozi[i].obracun.length;j++){
				for(var k=0;k<cenovnikHigh.length;k++){
					if(cenovnikHigh[k].code==nalozi[i].obracun[j].code){
						nalozi[i].obracun[j].cenaPodizvodjaca = parseFloat(nalozi[i].obracun[j].quantity)*parseFloat(cenovnikHigh[k].price);
						nalozi[i].obracun[j].naziv = cenovnikHigh[k].name;
						nalozi[i].obracun[j].unit = cenovnikHigh[k].unit;
						nalozi[i].obracun[j].price = cenovnikHigh[k].price;
						nalozi[i].ukupanIznosPodizvodjaca = nalozi[i].ukupanIznosPodizvodjaca + nalozi[i].obracun[j].cenaPodizvodjaca;
					}
				}
			}
		}
		fs.writeFileSync("nalozi.json",JSON.stringify(nalozi),{encoding:"utf8"});
		console.log("Struktura naloga aleksandar saric")*/
		/*for(var i=0;i<nalozi.length;i++){
			//1st line
			console.log("Broj naloga: "+nalozi[i].broj)
			console.log("Radna Jedinica: "+nalozi[i].radnaJedinica)
			console.log("Adresa: "+nalozi[i].adresa)
			//2nd line
			console.log("Opis naloga: "+nalozi[i].opis)
			//3rd line 
			console.log("Odradjene stavke")
			//4th line
			console.log("Naziv")
			console.log("Kolicina")
			console.log("Jedinica")
			console.log("Cena J/M")
			console.log("Ukupno")
			

			for(var j=0;j<nalozi[i].obracun.length;j++){
				//New line each loop
				console.log(nalozi[i].obracun[j].naziv)
				console.log(nalozi[i].obracun[j].quantity)
				console.log(nalozi[i].obracun[j].unit)
				console.log(nalozi[i].obracun[j].price)
				console.log(nalozi[i].obracun[j].cenaPodizvodjaca)
			}
			//new line
			console.log("Ukupna cena radova")
			//new line
			console.log(nalozi[i].ukupanIznosPodizvodjaca)
			//new line
			console.log("Zaduzen materijal")
			//new line
			for(var j=0;j<nalozi[i].reversi.length;j++){
				//New line each loop
				console.log("Broj reversa:" + nalozi[i].reversi[j].broj)
				//new line
				...
				console.log("Uzeto")
				console.log("Vraceno")
				console.log("Cena po jedinici mere")
				console.log("Ukupna cena")
				var cenaZaduzenja = 0;
				for(var k=0;k<nalozi[i].reversi[j].zaduzenje.length;k++){
					//new line each loop
					console.log(nalozi[i].reversi[j].zaduzenje[k].naziv)
					console.log(nalozi[i].reversi[j].zaduzenje[k].quantity+nalozi[i].reversi[j].zaduzenje[k].unit)
					console.log(nalozi[i].reversi[j].zaduzenje[k].quantity2+nalozi[i].reversi[j].zaduzenje[k].unit)
					console.log(nalozi[i].reversi[j].zaduzenje[k].price);
					console.log(parseFloat(nalozi[i].reversi[j].zaduzenje[k].price)*(parseFloat(nalozi[i].reversi[j].zaduzenje[k].quantity)-parseFloat(nalozi[i].reversi[j].zaduzenje[k].quantity2)))
					cenaZaduzenja = cenaZaduzenja + parseFloat(nalozi[i].reversi[j].zaduzenje[k].price)*(parseFloat(nalozi[i].reversi[j].zaduzenje[k].quantity)-parseFloat(nalozi[i].reversi[j].zaduzenje[k].quantity2));
				}
				//new line
				console.log("Ukupna cena materijala: "+cenaZaduzenja);
			}
		}*/
		//await exportNaloziReportOneSheet(nalozi, "nalozi_report.xlsx");


		//Za podizvodjace
		/*var opstine = ["STARI GRAD","SAVSKI VENAC","VRAČAR"];
		var nalozi = await naloziDB.find({statusNaloga:"Fakturisan",radnaJedinica:{$in:opstine}}).toArray();
		var woma	= ["80.02.09.003","80.02.09.004","80.02.09.020","80.02.09.021","80.02.09.022","80.02.09.023","80.02.09.024","80.02.09.025","80.02.09.026","80.02.09.027","80.02.09.028","80.02.09.029","80.04.01.006","80.02.09.031"];
		var crp   = ["80.02.09.007","80.02.09.008","80.02.09.009","80.02.09.010","80.02.09.011","80.02.09.012","80.02.09.013","80.02.09.014","80.02.09.015","80.02.09.016","80.02.09.017","80.02.09.018","80.04.01.012"];
		var kop 	= ["80.03.01.001","80.03.01.002","80.03.01.003","80.03.01.004","80.03.01.005","80.03.01.006","80.03.01.007","80.03.01.008","80.03.01.009","80.03.01.010","80.03.01.011","80.03.01.012","80.03.01.013","80.03.01.014","80.03.01.015","80.03.01.016","80.03.01.017","80.03.01.018","80.03.01.025","80.03.01.028","80.03.04.001","80.03.04.002","80.03.04.003","80.03.04.004","80.03.04.005","80.03.04.006","80.03.04.007","80.03.04.008","80.03.04.009","80.03.04.010","80.03.04.011","80.03.04.012","80.03.04.013","80.03.08.001","80.03.08.005","80.03.08.006","80.03.08.008","80.03.08.011"];
		var mas 	= ["80.03.01.019","80.03.01.020","80.03.01.021","80.03.01.022","80.03.01.023","80.03.01.024","80.03.01.026","80.03.01.027","80.03.01.029","80.03.01.030","80.03.04.014","80.03.04.015","80.03.04.023","80.03.04.024","80.03.04.028","80.03.04.030","80.03.04.032","80.03.04.037","80.03.04.038"];
		...
		var stavke = ["woma","crp","kop","mas","odgusenja","finalizacija","zamene"];
		...
		var meseci = [
				...
				{
					mesec: "08.2025",
					mesecIme: "Avgust 2025"
				},
				{
					mesec: "09.2025",
					mesecIme: "Septembar 2025"
				},
				{
					mesec: "10.2025",
					mesecIme: "Oktobar 2025"
				},
				{
					mesec: "11.2025",
					mesecIme: "Novembar 2025"
				},
				{
					mesec: "12.2025",
					mesecIme: "Decembar 2025"
				},
				{
					mesec: "01.2026",
					mesecIme: "Januar 2025"
				}
		];
		for(var i=0;i<meseci.length;i++){
			meseci[i].stavke = [];
			for(var j=0;j<stavke.length;j++){
				...
				json.iznos = 0;
				json.nalozi = 0;
				meseci[i].stavke.push(json);
			}
		}

		for(var i=0;i<nalozi.length;i++){
			
			for(var j=0;j<nalozi[i].stavke.length;j++){
		...
		// map mesec -> index (da ne vrtis svaki put)
		var meseciMap = {};
		for(var i=0;i<meseci.length;i++){
		  meseciMap[meseci[i].mesec] = i;
		}

		// map kategorija -> set kodova (mnogo brze od indexOf nad ogromnim nizovima)
		var kategorije = {
		  woma: new Set(woma),
		  crp: new Set(crp),
		  kop: new Set(kop),
		  mas: new Set(mas),
		  odgusenja: new Set(odgusenja),
		  finalizacija: new Set(finalizacija),
		  zamene: new Set(zamene),
		};

		function getKategorijaByCode(code){
		  if(woma.indexOf(code) >= 0) return "woma";
		  if(crp.indexOf(code) >= 0) return "crp";
		  if(kop.indexOf(code) >= 0) return "kop";
		  if(mas.indexOf(code) >= 0) return "mas";
		  if(odgusenja.indexOf(code) >= 0) return "odgusenja";
		  if(finalizacija.indexOf(code) >= 0) return "finalizacija";
		  if(zamene.indexOf(code) >= 0) return "zamene";
		  return null;
		}

		function getMesecKey(d){
		  if(!d) return null; // očekuje "DD.MM.YYYY"
		...
		for(var i=0;i<nalozi.length;i++){
		  ...
		  if(!nalozi[i].obracun || nalozi[i].obracun.length == 0) continue;
		  if(!nalozi[i].prijemnica || !nalozi[i].prijemnica.datum || !nalozi[i].prijemnica.datum.datum) continue;

		  var mesecKey = getMesecKey(nalozi[i].prijemnica.datum.datum);
		  var mesecIndex = meseciMap[mesecKey];
		  if(mesecIndex === undefined) continue;

		  // prati samo da nalog ne bude brojcan vise puta po kategoriji
		  var seenKategorije = {};

		  for(var j=0;j<nalozi[i].obracun.length;j++){

		    var code = nalozi[i].obracun[j].code;
		    if(!code) continue;

		    var kat = getKategorijaByCode(code);
		    if(kat == null) continue;

		    // parse price string
		    var price = 0;
		    if(nalozi[i].obracun[j].price != null){
		      price = parseFloat(String(nalozi[i].obracun[j].price).replace(",", ".")) || 0;
		    }

		    for(var k=0;k<meseci[mesecIndex].stavke.length;k++){
		      if(meseci[mesecIndex].stavke[k].naziv == kat){

		        // iznos SABIRAS UVEK (svaka stavka ima svoj price)
		        meseci[mesecIndex].stavke[k].iznos += price;

		        // nalozi brojis samo jednom po nalogu po kategoriji
		        if(seenKategorije[kat] !== true){
		          meseci[mesecIndex].stavke[k].nalozi += 1;
		          seenKategorije[kat] = true;
		        }
		...
		for(var i=0;i<meseci.length;i++){
		  meseci[i].ukupanIznos = 0;
		  meseci[i].ukupnoNaloga = 0;

		  for(var j=0;j<meseci[i].stavke.length;j++){
		    meseci[i].ukupanIznos += parseFloat(meseci[i].stavke[j].iznos) || 0;
		    meseci[i].ukupnoNaloga += parseInt(meseci[i].stavke[j].nalozi) || 0;
		  }

		  console.log(meseci[i].mesecIme);
		  console.log("UKUPNO IZNOS:", brojSaRazmacima(meseci[i].ukupanIznos));
		  console.log("UKUPNO NALOGA:", meseci[i].ukupnoNaloga);
		  console.log(meseci[i].stavke);
		}*/

		//maj 2024 stari ugovor i ceo novi
		/*var nalozi = await nalozi2024DB.find({majstor:{$in:podizvodjaci}}).toArray();//datum.datetime
		var cutOff = new Date("2024-05-01").getTime();
		var ukupanIznos = 0;
		var ukupnoNaloga = 0;

		for(var i=0;i<nalozi.length;i++){
			nalozi[i].iznosPodizvodjaca = 0;
			for(var j=0;j<nalozi[i].obracun.length;j++){
				for(var k=0;k<cenovnikHigh2024.length;k++){
					if(nalozi[i].obracun[j].code==cenovnikHigh2024[k].code){
						nalozi[i].iznosPodizvodjaca = nalozi[i].iznosPodizvodjaca + parseFloat(cenovnikHigh2024[k].price)*parseFloat(nalozi[i].obracun[j].quantity);
						break
					}
				}	
			}
		...
		for(var i=0;i<nalozi.length;i++){
			if(nalozi[i].datum.datetime>cutOff){
				if(!isNaN(parseFloat(nalozi[i].iznosPodizvodjaca))){
					ukupanIznos = ukupanIznos + parseFloat(nalozi[i].iznosPodizvodjaca);
					ukupnoNaloga++;
				}else{
					console.log(nalozi[i].iznosPodizvodjaca)
				}
			}
		}
		console.log("STARI UGOVOR PODIZVODJACI:")
		console.log("Iznos: "+brojSaRazmacima(ukupanIznos))
		console.log("Naloga: "+ukupnoNaloga)

		var nalozi = await naloziDB.find({majstor:{$in:podizvodjaci}}).toArray();//datum.datetime
		var ukupanIznos = 0;
		var ukupnoNaloga = 0;
		for(var i=0;i<nalozi.length;i++){
			nalozi[i].iznosPodizvodjaca = 0;
			for(var j=0;j<nalozi[i].obracun.length;j++){
				for(var k=0;k<cenovnikHigh.length;k++){
					if(nalozi[i].obracun[j].code==cenovnikHigh[k].code){
						nalozi[i].iznosPodizvodjaca = nalozi[i].iznosPodizvodjaca + parseFloat(cenovnikHigh[k].price)*parseFloat(nalozi[i].obracun[j].quantity);
						break
					}
				}	
			}
		...
		console.log("-------------------------")
		console.log("NOVI UGOVOR PODIZVODJACI:")
		console.log("Iznos: "+brojSaRazmacima(ukupanIznos))
		console.log("Naloga: "+ukupnoNaloga)*/
		...
		var nalozi = await naloziDB.find({broj:{$in:brojeviNaloga}}).toArray();
		for(var i=0;i<dodeljivaniNalozi.length;i++){
			dodeljivaniNalozi[i].nalogJson = {};
			for(var j=0;j<nalozi.length;j++){
				if(nalozi[j].broj==dodeljivaniNalozi[i].nalog){
					dodeljivaniNalozi[i].nalogJson = nalozi[j];
				}	
			}
		}
		...
		var sifraBager 					= ["80.03.01.019","80.03.01.020","80.03.01.021","80.03.01.022","80.03.01.023","80.03.01.024","80.03.01.026","80.03.01.027","80.03.01.029","80.03.01.030","80.03.04.014","80.03.04.015","80.03.04.023","80.03.04.024","80.03.04.028","80.03.04.030","80.03.04.032","80.03.04.037","80.03.04.038"];
		...
		for(var i=0;i<dodeljivaniNalozi.length;i++){
			var nalog = dodeljivaniNalozi[i].nalogJson;
			if(nalog.statusNaloga=="Spreman za fakturisanje" || nalog.statusNaloga=="Fakturisan"){
				zavrseno++;
				if(parseFloat(nalog.ukupanIznos) == 4600){
					lokalci++;
				}else{
					for(var j=0;j<nalog.obracun.length;j++){
						var cena = 0;
						for(var k=0;k<cenovnik.length;k++){
							if(cenovnik[k].code==nalog.obracun[j].code){
								cena = parseFloat(cenovnik[k].price);
								break;
							}
						}
						var iznos = nalog.obracun[j].quantity * cena;
						if(sifraWoma.indexOf(nalog.obracun[j].code)>=0){
							wome = wome + iznos;
						}else if(sifraCrpljenje.indexOf(nalog.obracun[j].code)>=0){
							crpljenje = crpljenje + iznos;
						}else if(sifraKopanje.indexOf(nalog.obracun[j].code)>=0){
							kopanje = kopanje + iznos
						}else if(sifraBager.indexOf(nalog.obracun[j].code)>=0){
							bager = bager + iznos
						}else if(sifraOdgusenja.indexOf(nalog.obracun[j].code)>=0){
							odgusenje = odgusenje + iznos
						}else if(sifraFinalizacija.indexOf(nalog.obracun[j].code)>=0){
							finalizacije = finalizacije + iznos
						}else if(sifraZamene.indexOf(nalog.obracun[j].code)>=0){
							zamene = zamene + iznos
						}
					}
				}
		...
		console.log("Ukupno naloga: " +dodeljivaniNalozi.length)
		console.log("Zavrseno: " +zavrseno)
		console.log("Nezavrseno: " +nezavrseno)
		console.log("Lokalni kvarovi: " +lokalci)
		console.log("------------------------------")
		console.log("Zamene: "+ brojSaRazmacima(zamene)+" RSD")
		console.log("Odgusenja: "+ brojSaRazmacima(odgusenje)+" RSD")
		console.log("Kopanje: "+ brojSaRazmacima(kopanje)+" RSD")
		console.log("Bager: "+ brojSaRazmacima(bager)+" RSD")
		console.log("Woma: "+ brojSaRazmacima(wome)+" RSD")
    ...
    console.log('✅ Excel file written');*/
   	...
   	var ukupno = 0;
   	var ukupnoPodizvodjaci = 0;
   	var ukupnoPG = 0;
   	for(var i=0;i<nalozi.length;i++){
   		ukupno = ukupno + parseFloat(nalozi[i].ukupanIznos)
   		if(podizvodjaci.indexOf(nalozi[i].majstor)>=0){
   			ukupnoPodizvodjaci = ukupnoPodizvodjaci + parseFloat(nalozi[i].ukupanIznos)
   		}else{
   			ukupnoPG = ukupnoPG + parseFloat(nalozi[i].ukupanIznos)
   		}
   	}
   	...
   	for(var w=0;w<radneJedinice.length;w++){
			...
			console.log(opseg[0] + " "+mesec+"/"+godina)
    ...
    var sifraKonstatacija 	=	["80.04.01.002","80.04.01.005"];
		...
		/*function getPreviousWeekDates(date = new Date()) {
		  ...
		  currentMonday.setDate(d.getDate() - day);
		  ...
		  for (let i = 0; i < 7; i++) {
		    const nextDate = new Date(previousMonday);
		    nextDate.setDate(previousMonday.getDate() + i);
		    dates.push(nextDate);
		  }
		...
		function getDatesBetween(startDate, endDate) {
		  ...
		  // Normalize time to avoid timezone/hour issues
		...
		function handleData(opseg,json,regionName){
			...
			if(regionName=="BEOGRAD"){
				for(var i=0;i<majstori.length;i++){
					json.ekipe = json.ekipe + majstori[i].cekiranja;
				}
				...
				for(var i=0;i<majstori.length;i++){
					if(majstori[i].region==regionName){
						json.ekipe = json.ekipe + majstori[i].cekiranja;
					}
		...
		var previousDates = getDatesBetween("2026-06-30", "2026-07-06");
		var previousDates2 = getDatesBetween("2026-06-30", "2026-07-06");
   	var query = getDateRangeQuery("2026-06-30", "2026-07-06");
		...
		//
   	var nalozi = await naloziDB.find({majstor:{$in:podizvodjaci},statusNaloga:{$in:["Fakturisan","Spreman za fakturisanje","Nalog u Stambenom"]},"prijemnica.datum.datum":{$in:previousDates}}).toArray();
   	var nalogIds = [];
   	for(var i=0;i<nalozi.length;i++){
   	...
   	for(var i=0;i<reversi.length;i++){
   		...
   		for(var j=0;j<reversi[i].zaduzenje.length;j++){
   			...
   			var quantity = parseFloat(reversi[i].zaduzenje[j].quantity) || 0;
				var quantity2 = parseFloat(reversi[i].zaduzenje[j].quantity2) || 0;
   			var kolicina = quantity - quantity2;
   			reversi[i].zaduzenje[j].cena = kolicina * cena;
   			reversi[i].iznosReversa = reversi[i].iznosReversa + kolicina*cena;
   		}
   	}

		const cekiranja = await checkInMajstoraDB.find(query).toArray();
		//console.log(cekiranja)
   	var majstori = await majstoriDB.find({}).toArray();
   	for (var i = 0; i < majstori.length; i++) {
		  majstori[i].cekiranja = 0;

		  var countedDays = new Set();

		  for (var j = 0; j < cekiranja.length; j++) {
		    if (cekiranja[j].brojKartice == majstori[i].brojKartice) {
		      var dayKey = cekiranja[j].year + "-" + cekiranja[j].month + "-" + cekiranja[j].date;

		      if (!countedDays.has(dayKey)) {
		        countedDays.add(dayKey);
		        majstori[i].cekiranja++;
		      }
		    }
		  }
		}
   	...
   	for(var i=0;i<naloziTrecihLica.length;i++){
   		for(var j=0;j<trecaLica.length;j++){
   			if(naloziTrecihLica[i].treceLice==trecaLica[j].uniqueId){
   				naloziTrecihLica[i].treceLiceNaziv = trecaLica[j].naziv;
   			}
   		}
   	}

   	naloziTrecihLica.sort((a, b) => {
		  const treceA = (a.treceLiceNaziv || "").localeCompare(b.treceLiceNaziv || "", "sr");
		  if (treceA !== 0) return treceA;

		  return (a.statusNaloga || "").localeCompare(b.statusNaloga || "", "sr");
		});
   	for(var i=0;i<naloziTrecihLica.length;i++){
   		naloziTrecihLica[i].region = "BEOGRAD";
   		var nalog = naloziTrecihLica[i];
   		console.log(nalog.treceLiceNaziv);
   		console.log(nalog.datumRadova);
   		console.log(nalog.opisNaloga);
   		console.log(brojSaRazmacima(nalog.ukupanIznos))
   		console.log(nalog.statusNaloga)
   		console.log("------------")
   		for(var j=0;j<majstori.length;j++){
   	...
   	fs.writeFileSync("beograd.json",JSON.stringify(json),{encoding:"utf8"})
		logResults("BEOGRAD",json)
		handleData(zapad,json,"ZAPAD")
   	fs.writeFileSync("zapad.json",JSON.stringify(json),{encoding:"utf8"})
		
		logResults("ZAPAD",json)
		handleData(istok,json,"ISTOK");

   	fs.writeFileSync("istok.json",JSON.stringify(json),{encoding:"utf8"})
		logResults("ISTOK",json)*/


		//STATISTIKA ZA PODCAST
		/*var godina = "2025";
   	var nalozi = await naloziDB.find({statusNaloga:"Fakturisan","prijemnica.datum.datum":{$regex:"."+godina}}).toArray()
    ...
    var sifraKonstatacija 	=	["80.04.01.002","80.04.01.005"];
		...
		json.finalizacijaBroj = 0;
		json.zameneBroj = 0;
		json.ukupnoNaloga = nalozi.length;
		for(var i=0;i<nalozi.length;i++){
			var nalog = nalozi[i];
			for(var j=0;j<nalozi[i].obracun.length;j++){
				if(sifraKonstatacija.indexOf(nalozi[i].obracun[j].code)>=0){
		...
		console.log("UKUPNO")
		console.log("  Naloga: "+json.ukupnoNaloga)
		console.log("  Zagusenja: "+json.odgusenjaBroj)
		console.log("  Crpljenje: "+json.crpljenjeBroj)
		console.log("  Vome: "+json.womaBroj)
		console.log("  Rovovi: "+json.bagerBroj)
		...
		/*var godina = "2024";
   	var nalozi = await nalozi2024DB.find({statusNaloga:"Fakturisan","prijemnica.datum.datum":{$regex:"."+godina}}).toArray()

    var sifraKonstatacija 	=	["80.04.01.002","80.04.01.004","80.04.01.005","80.04.01.006"];
		var sifraWoma						= ["80.02.09.020","80.02.09.021","80.02.09.022","80.02.09.023","80.02.09.024"];;
		var sifraCrpljenje 			= ["80.02.09.007", "80.02.09.008", "80.02.09.009", "80.02.09.010", "80.02.09.011", "80.02.09.012", "80.02.09.013", "80.02.09.014", "80.02.09.015", "80.02.09.016", "80.02.09.017", "80.02.09.018", "80.02.09.019", "80.02.09.025", "80.02.09.026", "80.02.09.027", "80.02.09.028", "80.02.09.029"];
		var sifraKopanje 				= ["80.03.01.001","80.03.01.002","80.03.01.003","80.03.01.004","80.03.01.005","80.03.01.006","80.03.01.007","80.03.01.008","80.03.01.009","80.03.01.010","80.03.01.011","80.03.01.012","80.03.01.013","80.03.01.014","80.03.01.015","80.03.01.016","80.03.01.017","80.03.01.018","80.03.01.028"];
		var sifraBager					= ["80.03.01.019", "80.03.01.020", "80.03.01.021", "80.03.01.022", "80.03.01.023", "80.03.01.024"];
		var sifraOdgusenja 			= ["80.02.09.001","80.02.09.002","80.02.09.003","80.02.09.004","80.02.09.005"];
		var sifraFinalizacija 	= ["80.03.01.025","80.03.01.026","80.03.01.027","80.03.02.001","80.03.02.002","80.03.02.003","80.03.02.004","80.03.02.005","80.03.02.006","80.03.02.007","80.03.02.008","80.03.02.009","80.03.02.010","80.03.02.011","80.03.02.012","80.03.02.013","80.03.02.014","80.03.02.015","80.03.02.016","80.03.03.001","80.03.03.002","80.03.03.003","80.03.03.004","80.03.03.005","80.03.03.006","80.03.03.007","80.03.03.008","80.03.03.009","80.03.03.014","80.03.03.015","80.03.03.016","80.03.03.017","80.03.03.018","80.03.03.019","80.03.03.020","80.03.03.021","80.03.03.022","80.03.03.023","80.03.03.024","80.03.03.025","80.03.03.026","80.03.03.027","80.03.03.028","80.03.03.029","80.03.03.030","80.03.03.031","80.03.03.032","80.03.03.033","80.03.03.034","80.03.03.035","80.03.03.036","80.03.03.037","80.03.03.038","80.03.03.039","80.03.03.040","80.03.03.041","80.03.03.042","80.03.03.043","80.03.03.044","80.03.03.045","80.03.03.046","80.03.03.047","80.03.03.048","80.03.03.049","80.03.03.050","80.03.03.051","80.03.03.052","80.03.03.053","80.03.03.054","80.03.03.055","80.03.03.056","80.03.03.057","80.03.03.058","80.03.03.059","80.03.03.060","80.03.03.061","80.03.03.062","80.03.03.063","80.03.03.064","80.03.03.065","80.03.03.066","80.03.03.067","80.03.03.068","80.03.03.069","80.03.03.070","80.03.03.071","80.03.03.072","80.03.03.073","80.03.03.074","80.03.03.075","80.03.03.076","80.03.03.077","80.03.03.078","80.03.03.079","80.03.03.080","80.03.03.081","80.03.03.082","80.03.03.083","80.03.03.084","80.03.03.085","80.03.03.086","80.03.03.087","80.03.03.088","80.03.03.089","80.03.03.090","80.03.03.091","80.03.03.092","80.03.03.093","80.03.03.094","80.03.03.095","80.03.03.096","80.03.03.097","80.03.03.098","80.03.03.099","80.03.03.100","80.03.03.101","80.03.03.102","80.03.03.103","80.03.03.104","80.03.03.105","80.03.03.106","80.03.03.107","80.03.03.108","80.03.03.109","80.03.03.110","80.03.03.111","80.03.03.112","80.03.03.113","80.03.03.114","80.03.03.115","80.03.03.116","80.03.03.117","80.03.03.118","80.03.03.119","80.03.03.120","80.03.03.121","80.03.03.122","80.03.03.123","80.03.03.124","80.03.03.125","80.03.03.126","80.03.03.127","80.03.03.128","80.03.03.129","80.03.03.130","80.03.03.131","80.03.03.132","80.03.03.133","80.03.03.134","80.03.03.135","80.03.03.136","80.03.03.137","80.03.03.138","80.03.04.001","80.03.04.002","80.03.04.003","80.03.04.004","80.03.04.005","80.03.04.006","80.03.04.007","80.03.04.008","80.03.04.009","80.03.04.010","80.03.04.011","80.03.04.012","80.03.04.013","80.03.04.014","80.03.04.015","80.03.04.016","80.03.04.017","80.03.04.018","80.03.04.019","80.03.04.020","80.03.04.021","80.03.04.022","80.03.04.023","80.03.04.024","80.03.04.025","80.03.04.026","80.03.04.027","80.03.04.028","80.03.04.029","80.03.04.030","80.03.04.031","80.03.04.032","80.03.04.033","80.03.04.034","80.03.04.035","80.03.05.001","80.03.05.002","80.03.05.003","80.03.05.004","80.03.05.005","80.03.05.006","80.03.05.007","80.03.05.008","80.03.05.009","80.03.05.010","80.03.05.011","80.03.05.012","80.03.05.013","80.03.06.001","80.03.06.002","80.03.06.003","80.03.06.004","80.03.06.005","80.03.06.006","80.03.06.007","80.03.06.008","80.03.06.009","80.03.06.010","80.03.06.011","80.03.06.012","80.03.06.013","80.03.06.014","80.03.07.001","80.03.07.002","80.03.07.003","80.03.07.004","80.03.07.005","80.03.07.006","80.03.07.007","80.03.07.008","80.03.07.009","80.03.07.010","80.03.07.011","80.03.07.012","80.03.07.013","80.03.07.014","80.03.07.015","80.03.07.016","80.03.07.017","80.03.07.018","80.03.07.019","80.03.07.020","80.03.07.021","80.03.07.022","80.03.07.023","80.03.08.001","80.03.08.002","80.03.08.003","80.03.08.004","80.03.08.005","80.03.08.006","80.03.08.007","80.03.08.008","80.03.08.009","80.03.08.010","80.03.08.011","80.03.08.012","80.03.08.013","80.03.09.001","80.03.09.002","80.03.09.003","80.03.09.004","80.03.09.005","80.03.09.006","80.03.09.007","80.03.09.008","80.03.09.009","80.04.01.001","80.04.01.003","80.04.01.007"];
		var sifraZamene 				= ["80.01.01.001","80.01.01.002","80.01.01.003","80.01.01.004","80.01.01.005","80.01.01.006","80.01.01.007","80.01.01.008","80.01.01.009","80.01.01.010","80.01.01.011","80.01.01.012","80.01.01.013","80.01.01.014","80.01.01.015","80.01.01.016","80.01.02.001","80.01.02.002","80.01.02.003","80.01.02.004","80.01.02.005","80.01.02.006","80.01.02.007","80.01.02.008","80.01.02.029","80.01.02.030","80.01.02.031","80.01.02.032","80.01.02.033","80.01.02.034","80.01.02.035","80.01.02.036","80.01.02.056","80.01.02.057","80.01.02.058","80.01.02.059","80.01.02.060","80.01.02.061","80.01.02.062","80.01.02.063","80.01.02.085","80.01.02.086","80.01.02.087","80.01.02.088","80.01.02.089","80.01.02.090","80.01.02.091","80.01.02.092","80.01.02.114","80.01.02.115","80.01.02.116","80.01.02.117","80.01.02.118","80.01.02.119","80.01.02.120","80.01.02.121","80.01.02.122","80.01.02.123","80.01.02.124","80.01.02.125","80.01.02.126","80.01.02.127","80.01.02.128","80.01.02.129","80.01.02.130","80.01.02.131","80.01.02.132","80.01.02.133","80.01.02.134","80.01.02.135","80.01.02.136","80.01.02.137","80.01.02.138","80.01.02.194","80.01.02.195","80.01.02.196","80.01.02.197","80.01.02.198","80.01.02.199","80.01.02.200","80.01.02.201","80.01.03.001","80.01.03.002","80.01.03.003","80.01.03.004","80.01.03.005","80.01.03.006","80.01.03.007","80.01.03.008","80.01.03.009","80.01.03.010","80.01.03.011","80.01.03.012","80.01.03.013","80.01.03.014","80.01.03.015","80.01.03.016","80.01.04.001","80.01.04.002","80.01.04.003","80.01.04.004","80.01.04.005","80.01.04.006","80.01.04.007","80.01.04.008","80.01.04.009","80.01.05.001","80.01.05.002","80.01.05.003","80.01.05.004","80.01.05.005","80.01.05.006","80.01.05.007","80.01.05.008","80.01.05.009","80.01.05.010","80.01.05.011","80.01.05.012","80.01.05.013","80.01.05.014","80.01.05.015","80.01.05.016","80.01.05.017","80.01.05.018","80.01.05.019","80.01.05.020","80.01.05.021","80.01.05.022","80.01.05.023","80.01.05.024","80.01.05.025","80.01.05.026","80.01.05.027","80.01.05.028","80.01.05.029","80.01.05.030","80.01.05.031","80.01.05.032","80.01.05.033","80.01.05.034","80.01.05.035","80.01.05.036","80.01.05.037","80.01.05.038","80.01.05.039","80.01.05.040","80.01.05.041","80.01.05.042","80.01.05.043","80.01.05.044","80.01.05.045","80.01.05.046","80.01.05.047","80.01.05.048","80.01.05.049","80.01.05.050","80.01.05.051","80.01.05.052","80.01.05.053","80.01.05.054","80.01.05.055","80.01.05.056","80.01.05.057","80.01.05.058","80.01.05.059","80.01.05.060","80.01.05.061","80.01.05.062","80.01.05.063","80.01.05.064","80.01.05.065","80.01.05.066","80.01.05.067","80.01.05.068","80.01.05.069","80.01.05.070","80.01.05.071","80.01.05.072","80.01.05.073","80.01.05.074","80.01.05.075","80.01.05.076","80.01.05.077","80.01.05.078","80.01.05.079","80.01.05.080","80.01.05.081","80.01.05.082","80.01.05.083","80.01.05.084","80.01.05.085","80.01.05.086","80.01.05.087","80.01.05.088","80.01.05.089","80.01.05.090","80.01.05.091","80.01.05.092","80.01.05.093","80.01.05.094","80.01.05.095","80.01.05.096","80.01.06.001","80.01.06.002","80.01.06.003","80.01.06.004","80.01.06.005","80.01.06.006","80.01.06.007","80.01.06.008","80.01.06.009","80.01.06.010","80.01.06.011","80.01.06.012","80.01.06.013","80.01.06.014","80.01.06.015","80.01.06.016","80.01.06.017","80.01.06.018","80.01.06.019","80.01.06.020","80.01.06.021","80.01.06.022","80.01.06.023","80.01.06.024","80.01.06.025","80.01.06.026","80.01.06.027","80.01.06.028","80.01.06.029","80.01.06.030","80.01.06.031","80.01.06.032","80.01.06.033","80.01.06.034","80.01.06.035","80.01.06.036","80.01.06.037","80.01.06.038","80.01.06.039","80.01.06.040","80.01.06.041","80.01.06.042","80.01.06.043","80.01.06.044","80.01.06.045","80.01.06.046","80.01.06.047","80.01.06.048","80.01.06.049","80.01.06.050","80.01.06.051","80.01.06.052","80.01.06.053","80.01.06.054","80.01.06.055","80.01.06.056","80.01.07.001","80.01.07.002","80.01.07.003","80.01.07.004","80.01.07.005","80.01.07.006","80.01.07.007","80.01.07.008","80.01.07.009","80.01.07.010","80.01.07.011","80.01.07.012","80.01.07.013","80.01.07.014","80.01.07.015","80.01.07.016","80.01.07.017","80.01.07.018","80.01.07.019","80.01.07.020","80.01.07.021","80.01.07.022","80.01.07.023","80.01.07.024","80.01.08.001","80.01.08.002","80.01.08.003","80.01.08.004","80.01.08.005","80.01.08.006","80.01.08.007","80.01.08.008","80.01.08.009","80.01.08.010","80.01.08.011","80.01.08.012","80.01.08.013","80.01.08.014","80.01.08.015","80.01.08.016","80.01.08.017","80.01.08.018","80.01.08.019","80.01.08.020","80.01.08.021","80.01.08.022","80.01.08.023","80.01.08.024","80.01.08.025","80.01.08.026","80.01.08.027","80.01.08.028","80.01.08.029","80.01.08.030","80.01.08.031","80.01.08.032","80.01.08.033","80.01.08.034","80.01.08.035","80.01.08.036","80.01.08.037","80.01.08.038","80.01.08.039","80.01.08.040","80.01.08.041","80.01.08.042","80.01.08.043","80.01.08.044","80.01.08.045","80.01.08.046","80.01.08.047","80.01.08.048","80.01.08.049","80.01.08.050","80.01.08.051","80.01.08.057","80.01.08.058","80.01.08.059","80.01.08.065","80.01.08.066","80.01.08.067","80.01.08.073","80.01.08.074","80.01.08.075","80.01.08.076","80.01.08.077","80.01.08.078","80.01.08.079","80.01.08.080","80.01.08.081","80.01.08.082","80.01.08.083","80.01.08.084","80.01.08.085","80.01.08.086","80.01.08.087","80.01.08.088","80.01.08.089","80.01.08.090","80.01.08.091","80.01.08.092","80.01.08.093","80.01.08.094","80.01.08.095","80.01.08.096","80.01.08.097","80.01.08.098","80.01.08.099","80.01.08.100","80.01.08.101","80.01.08.102","80.01.08.103","80.01.08.104","80.01.08.105","80.01.08.106","80.01.08.107","80.01.08.108","80.01.08.109","80.01.08.110","80.01.08.111","80.01.08.112","80.01.08.113","80.01.08.114","80.01.08.115","80.01.08.116","80.01.08.117","80.01.08.118","80.01.08.119","80.01.08.120","80.01.08.121","80.01.08.122","80.01.08.123","80.01.08.124","80.01.08.125","80.01.08.126","80.01.08.127","80.01.08.128","80.01.08.129","80.01.08.130","80.01.08.131","80.01.08.132","80.01.08.133","80.01.08.134","80.01.08.135","80.01.08.136","80.01.08.137","80.01.08.138","80.01.08.139","80.01.08.140","80.01.08.141","80.01.08.142","80.01.08.143","80.01.08.144","80.01.08.145","80.01.08.146","80.01.08.147","80.01.08.153","80.01.08.154","80.01.08.155","80.01.08.161","80.01.08.162","80.01.08.163","80.01.08.170","80.01.08.171","80.01.08.172","80.01.08.173","80.01.08.174","80.01.08.175","80.01.08.176","80.01.08.178","80.01.08.179","80.01.08.180","80.01.08.181","80.01.08.182","80.01.08.183","80.01.08.184","80.01.08.186","80.01.08.187","80.01.08.188","80.01.08.189","80.01.08.190","80.01.08.191","80.01.08.192","80.01.08.193","80.01.08.194","80.01.08.195","80.01.08.196","80.01.08.197","80.01.08.198","80.01.08.199","80.01.08.200","80.01.08.201","80.01.08.202","80.01.08.203","80.01.08.204","80.01.08.205","80.01.08.206","80.01.08.207","80.01.08.208","80.01.08.209","80.01.08.210","80.01.08.211","80.01.08.212","80.01.08.213","80.01.08.214","80.01.08.215","80.01.08.216","80.01.09.001","80.01.09.002","80.01.09.003","80.01.09.004","80.01.09.005","80.01.09.006","80.01.09.007","80.01.09.008","80.01.09.009","80.01.09.010","80.01.09.011","80.01.09.012","80.01.09.013","80.01.09.014","80.01.09.015","80.01.09.016","80.01.09.017","80.01.09.018","80.01.09.019","80.01.09.020","80.01.09.021","80.01.09.022","80.01.09.023","80.01.09.024","80.01.10.001","80.01.10.002","80.01.10.003","80.01.10.004","80.01.10.005","80.01.10.006","80.01.10.007","80.01.10.008","80.01.10.009","80.01.10.010","80.01.10.011","80.01.10.012","80.01.10.013","80.01.10.014","80.01.10.015","80.01.10.016","80.01.10.017","80.01.10.018","80.01.10.019","80.01.10.020","80.01.10.021","80.01.10.022","80.01.10.023","80.01.10.024","80.01.11.001","80.01.11.002","80.01.11.003","80.01.11.004","80.01.11.005","80.01.11.006","80.01.11.007","80.01.11.008","80.01.11.009","80.01.11.010","80.01.11.011","80.01.11.012","80.01.11.013","80.01.11.014","80.01.11.015","80.01.11.016","80.01.11.017","80.01.11.018","80.01.11.019","80.01.11.020","80.01.11.021","80.01.11.022","80.01.11.023","80.01.11.024","80.01.12.001","80.01.12.002","80.01.12.003","80.01.12.004","80.01.12.005","80.01.12.006","80.01.12.007","80.01.12.008","80.01.13.001","80.01.13.002","80.01.13.003","80.01.13.004","80.01.13.005","80.01.13.006","80.01.13.007","80.01.13.008","80.01.13.009","80.01.13.010","80.01.13.011","80.01.13.012","80.01.13.013","80.01.13.014","80.01.13.015","80.01.13.016","80.01.13.017","80.01.13.018","80.01.13.019","80.01.13.020","80.01.13.021","80.01.13.022","80.01.13.023","80.01.13.024","80.01.14.001","80.01.14.002","80.01.14.003","80.01.14.004","80.01.14.005","80.01.14.006","80.01.14.007","80.01.14.008","80.01.14.009","80.01.14.010","80.01.14.011","80.01.14.012","80.01.14.013","80.01.14.014","80.01.14.015","80.01.14.016","80.01.14.017","80.01.14.018","80.01.14.019","80.01.14.020","80.01.14.021","80.01.14.022","80.01.14.023","80.01.14.024","80.01.14.025","80.01.14.026","80.01.14.027","80.01.14.028","80.01.14.029","80.01.14.030","80.01.14.031","80.01.14.032","80.01.14.033","80.01.14.034","80.01.14.035","80.01.14.036","80.01.14.037","80.01.14.038","80.01.14.039","80.01.14.040","80.01.14.041","80.01.14.042","80.01.14.043","80.01.14.044","80.01.14.045","80.01.14.046","80.01.14.047","80.01.14.048","80.01.15.001","80.01.15.002","80.01.15.003","80.01.15.004","80.01.15.005","80.01.15.006","80.01.15.007","80.01.15.008","80.01.15.009","80.01.15.010","80.01.15.011","80.01.15.012","80.01.16.001","80.01.16.002","80.01.16.003","80.01.16.004","80.01.16.005","80.02.01.001","80.02.01.002","80.02.01.003","80.02.01.004","80.02.01.005","80.02.01.006","80.02.01.007","80.02.01.008","80.02.01.009","80.02.01.010","80.02.01.011","80.02.01.012","80.02.01.013","80.02.01.014","80.02.01.015","80.02.01.016","80.02.01.017","80.02.01.018","80.02.01.019","80.02.01.020","80.02.01.021","80.02.01.022","80.02.01.023","80.02.01.024","80.02.01.025","80.02.01.026","80.02.01.027","80.02.01.028","80.02.01.029","80.02.01.030","80.02.01.031","80.02.01.032","80.02.01.033","80.02.01.034","80.02.01.035","80.02.01.036","80.02.01.037","80.02.01.038","80.02.01.039","80.02.01.040","80.02.02.001","80.02.02.002","80.02.02.003","80.02.02.004","80.02.02.005","80.02.02.006","80.02.02.007","80.02.02.008","80.02.02.009","80.02.02.010","80.02.02.011","80.02.02.012","80.02.02.013","80.02.02.014","80.02.02.015","80.02.02.016","80.02.02.017","80.02.02.018","80.02.02.019","80.02.02.020","80.02.02.021","80.02.02.022","80.02.02.023","80.02.02.024","80.02.02.025","80.02.02.026","80.02.02.027","80.02.02.028","80.02.02.029","80.02.02.030","80.02.02.031","80.02.02.032","80.02.02.033","80.02.02.034","80.02.02.035","80.02.02.036","80.02.02.037","80.02.02.038","80.02.02.039","80.02.02.040","80.02.03.001","80.02.03.002","80.02.03.003","80.02.03.004","80.02.03.005","80.02.03.006","80.02.03.007","80.02.03.008","80.02.03.009","80.02.03.010","80.02.03.011","80.02.03.012","80.02.03.013","80.02.03.014","80.02.03.015","80.02.03.016","80.02.03.017","80.02.03.018","80.02.03.019","80.02.03.020","80.02.03.021","80.02.03.022","80.02.03.023","80.02.03.024","80.02.03.025","80.02.03.026","80.02.03.027","80.02.03.028","80.02.03.029","80.02.03.030","80.02.03.031","80.02.03.032","80.02.03.033","80.02.03.034","80.02.03.035","80.02.03.036","80.02.03.037","80.02.03.038","80.02.03.039","80.02.03.040","80.02.03.041","80.02.03.042","80.02.03.043","80.02.03.044","80.02.03.045","80.02.03.046","80.02.03.047","80.02.03.048","80.02.03.049","80.02.03.050","80.02.03.051","80.02.03.052","80.02.03.053","80.02.03.054","80.02.03.055","80.02.03.056","80.02.03.057","80.02.03.058","80.02.03.059","80.02.03.060","80.02.03.061","80.02.03.062","80.02.03.063","80.02.03.064","80.02.03.065","80.02.03.066","80.02.03.067","80.02.03.068","80.02.03.069","80.02.03.070","80.02.03.071","80.02.03.072","80.02.04.001","80.02.04.002","80.02.04.003","80.02.04.004","80.02.04.005","80.02.04.006","80.02.04.007","80.02.04.008","80.02.04.009","80.02.04.010","80.02.04.011","80.02.04.012","80.02.04.013","80.02.04.014","80.02.04.015","80.02.04.016","80.02.04.017","80.02.04.018","80.02.04.019","80.02.04.020","80.02.04.021","80.02.04.022","80.02.04.023","80.02.04.024","80.02.04.025","80.02.04.026","80.02.04.027","80.02.04.028","80.02.04.029","80.02.04.030","80.02.04.031","80.02.04.032","80.02.04.033","80.02.04.034","80.02.04.035","80.02.04.036","80.02.04.037","80.02.04.038","80.02.04.039","80.02.04.040","80.02.04.041","80.02.04.042","80.02.04.043","80.02.04.044","80.02.04.045","80.02.04.046","80.02.04.047","80.02.04.048","80.02.04.049","80.02.04.050","80.02.04.051","80.02.04.052","80.02.04.053","80.02.04.054","80.02.04.055","80.02.04.056","80.02.04.057","80.02.04.058","80.02.04.059","80.02.04.060","80.02.04.061","80.02.04.062","80.02.04.063","80.02.04.064","80.02.04.065","80.02.04.066","80.02.04.067","80.02.04.068","80.02.04.069","80.02.04.070","80.02.04.071","80.02.04.072","80.02.04.073","80.02.04.074","80.02.04.075","80.02.04.076","80.02.04.077","80.02.04.078","80.02.04.079","80.02.04.080","80.02.04.081","80.02.04.082","80.02.04.083","80.02.04.084","80.02.04.085","80.02.04.086","80.02.04.087","80.02.04.088","80.02.04.089","80.02.04.090","80.02.04.091","80.02.04.092","80.02.04.093","80.02.04.094","80.02.04.095","80.02.04.096","80.02.04.097","80.02.04.098","80.02.04.099","80.02.04.100","80.02.04.101","80.02.04.102","80.02.04.103","80.02.04.104","80.02.04.105","80.02.04.106","80.02.04.107","80.02.04.108","80.02.04.109","80.02.04.110","80.02.04.111","80.02.04.112","80.02.04.113","80.02.04.114","80.02.04.115","80.02.04.116","80.02.04.117","80.02.04.118","80.02.04.119","80.02.04.121","80.02.04.122","80.02.04.123","80.02.04.124","80.02.04.125","80.02.04.126","80.02.04.127","80.02.04.129","80.02.04.130","80.02.04.131","80.02.04.132","80.02.04.133","80.02.04.134","80.02.04.135","80.02.04.137","80.02.04.138","80.02.04.139","80.02.04.140","80.02.04.141","80.02.04.142","80.02.04.143","80.02.04.144","80.02.04.145","80.02.04.146","80.02.04.147","80.02.04.148","80.02.04.149","80.02.04.150","80.02.04.151","80.02.04.152","80.02.04.153","80.02.04.154","80.02.04.155","80.02.04.156","80.02.04.157","80.02.04.158","80.02.04.159","80.02.04.160","80.02.04.161","80.02.04.162","80.02.04.163","80.02.04.164","80.02.04.165","80.02.04.166","80.02.04.167","80.02.04.168","80.02.04.169","80.02.04.170","80.02.04.171","80.02.04.172","80.02.04.173","80.02.04.174","80.02.04.175","80.02.04.176","80.02.04.177","80.02.04.178","80.02.04.179","80.02.04.180","80.02.04.181","80.02.04.182","80.02.04.183","80.02.04.184","80.02.04.185","80.02.04.186","80.02.04.187","80.02.04.188","80.02.04.189","80.02.04.190","80.02.04.191","80.02.04.192","80.02.04.193","80.02.04.194","80.02.04.195","80.02.04.196","80.02.04.197","80.02.04.198","80.02.04.199","80.02.04.200","80.02.04.201","80.02.04.202","80.02.04.203","80.02.04.204","80.02.04.205","80.02.04.206","80.02.04.207","80.02.04.208","80.02.04.209","80.02.04.210","80.02.04.211","80.02.04.212","80.02.04.213","80.02.04.214","80.02.04.215","80.02.04.216","80.02.04.217","80.02.04.218","80.02.04.219","80.02.04.220","80.02.04.221","80.02.04.222","80.02.04.223","80.02.04.224","80.02.04.225","80.02.04.226","80.02.04.227","80.02.04.228","80.02.04.229","80.02.04.230","80.02.04.231","80.02.04.232","80.02.04.233","80.02.04.234","80.02.04.235","80.02.04.236","80.02.04.237","80.02.04.238","80.02.04.239","80.02.04.240","80.02.04.241","80.02.04.242","80.02.04.243","80.02.04.244","80.02.04.245","80.02.04.246","80.02.04.247","80.02.04.248","80.02.04.249","80.02.04.250","80.02.04.251","80.02.04.252","80.02.04.253","80.02.04.254","80.02.04.255","80.02.04.256","80.02.04.257","80.02.04.258","80.02.04.259","80.02.04.260","80.02.04.261","80.02.04.262","80.02.04.263","80.02.04.264","80.02.04.265","80.02.05.001","80.02.05.002","80.02.05.003","80.02.05.004","80.02.05.005","80.02.05.006","80.02.05.007","80.02.05.008","80.02.05.009","80.02.05.010","80.02.05.011","80.02.05.012","80.02.05.013","80.02.05.014","80.02.05.015","80.02.06.001","80.02.06.002","80.02.06.003","80.02.06.004","80.02.06.005","80.02.06.006","80.02.07.001","80.02.07.002","80.02.07.003","80.02.07.004","80.02.07.005","80.02.08.001","80.02.08.002","80.02.08.003","80.02.08.004","80.02.08.005","80.02.08.006","80.02.08.007","80.02.08.008","80.02.08.009","80.02.08.010","80.02.08.011","80.02.08.012","80.02.09.030","80.02.10.001","80.02.10.002","80.02.10.003","80.02.10.004","80.02.10.005","80.02.10.006","80.02.10.007","80.02.10.008","80.02.11.001","80.02.11.005","80.02.11.009","80.02.11.020","80.02.11.021","80.02.11.022","80.02.11.023","80.02.11.024","80.02.11.025","80.02.11.026","80.02.11.027","80.02.11.028","80.02.11.029","80.02.11.030","80.02.11.040","80.02.11.041","80.02.11.042","80.02.11.043","80.02.11.044","80.02.11.045","80.02.11.046","80.02.11.047","80.02.11.048","80.02.11.060","80.02.11.061","80.02.11.062","80.02.11.063","80.02.11.064","80.02.11.065","80.02.11.066","80.02.11.067","80.02.11.068","80.02.11.080","80.02.11.081","80.02.11.082","80.02.11.083","80.02.11.084","80.02.11.085","80.02.11.090","80.02.11.091","80.02.11.092","80.02.11.100","80.02.11.101","80.02.11.102","80.02.11.110","80.02.11.120","80.03.03.010","80.03.03.011","80.03.03.012","80.03.03.013","80.04.01.008"];
...
function borderNalog(ws, startRow, endRow) {
  ...
  for (let r = startRow; r <= endRow; r++) {
    for (let c = 1; c <= 5; c++) {
      ...
      cell.border = {
        top:    r === startRow ? thick : cell.border?.top,
        bottom: r === endRow   ? thick : cell.border?.bottom,
        left:   c === 1        ? thick : thin,
        right:  c === 5        ? thick : thin,
      };
    }
  }
}

async function exportNaloziReportOneSheet(nalozi, outFile = "nalozi_report.xlsx") {
	const totalRadoviRows = [];     // E row numbers
const totalMaterijalRows = [];
  const wb = new ExcelJS.Workbook();
  const ws = wb.addWorksheet("Izvestaj", {
  ...
  function addRow5(values) {
    const row = ws.addRow(values);
    row.alignment = { vertical: "middle", wrapText: true };
    return row;
  }

  function fullWidth(text, opts = {}) {
    const r = addRow5([text, "", "", "", ""]);
    ws.mergeCells(`A${r.number}:E${r.number}`);
    r.getCell(1).font = opts.font ?? { bold: true };
    if (opts.fill) r.getCell(1).fill = opts.fill;
    return r;
  }

  function labelValue(label, value) {
    const r = addRow5([label, "", value, "", ""]);
  ...
  function labelValueAE(label, valueCellSetter) {
  // A:D merged label, E is value (number/formula)
	  const r = addRow5([label, "", "", "", ""]);
	  ws.mergeCells(`A${r.number}:D${r.number}`);
	  r.getCell(1).font = { bold: true };

	  // pass BOTH cell and rowNumber
	  valueCellSetter(r.getCell(5), r.number);

	  return r;
	}

  function thinBorderAll(row) {
    const border = {
      top: { style: "thin" },
      left: { style: "thin" },
      bottom: { style: "thin" },
  ...
  // Radovi (5 columns)
  function headerRadovi() {
    const r = addRow5(["Naziv", "Kolicina", "Jedinica", "Cena J/M", "Ukupno"]);
    r.font = { bold: true };
    thinBorderAll(r);
  }

  function rowRadovi(st) {
	  const r = ws.addRow([
	    s(st?.naziv),        // A
	    n(st?.quantity),     // B  (Kolicina)
	    s(st?.unit),         // C  (Jedinica)
	    n(st?.price),        // D  (Cena J/M)
	    null                 // E  (Ukupno – formula)
	  ]);

	  // Ukupno = Kolicina * Cena J/M
	  r.getCell(5).value = {
	    formula: `B${r.number}*D${r.number}`
	  };
  ...
  // Materijal (5 columns)
  function headerMaterijal() {
	  const r = addRow5(["Stavka", "Uzeto (neto)", "Jedinica mere", "Cena po JM", "Ukupna cena"]);
	  r.font = { bold: true };
	  thinBorderAll(r);
	}

	function rowMaterijal(z) {
	  const r = addRow5([
	    s(z?.naziv),        // A
	    null,               // B -> formula: quantity - quantity2
	    s(z?.unit),         // C
	    n(z?.price),        // D
	    null                // E -> formula: B * D
	  ]);
	  ...
	  // B = Uzeto - Vraceno
	  r.getCell(2).value = { formula: `${n(z?.quantity)}-${n(z?.quantity2)}` };
	  r.getCell(2).numFmt = "0.00";

	  // E = B * D
	  r.getCell(5).value = { formula: `B${r.number}*D${r.number}` };
	  r.getCell(4).numFmt = "#,##0.00";
	  r.getCell(5).numFmt = "#,##0.00";

	  thinBorderAll(r);
	  return r.number; // important for SUM ranges
	}


  // Title
  const title = addRow5(["Struktura naloga aleksandar saric", "", "", "", ""]);
  ...
  for (const nalog of nalozi) {
  	const nalogStartRow = ws.lastRow.number + 1;
    fullWidth(`Broj naloga: ${s(nalog?.broj)}`, {
      font: { bold: true, size: 12 },
      fill: { type: "pattern", pattern: "solid", fgColor: { argb: "FFEFEFEF" } },
    });

    labelValue("Radna jedinica", s(nalog?.radnaJedinica));
    labelValue("Adresa", s(nalog?.adresa));
    labelValue("Opis naloga", s(nalog?.opis));

    // ---------------- RADOVI ----------------
    fullWidth("Odradjene stavke", {
      fill: { type: "pattern", pattern: "solid", fgColor: { argb: "FFF7F7F7" } },
    });

    headerRadovi();

    const obracun = Array.isArray(nalog?.obracun) ? nalog.obracun : [];
    let radoviStartRow = null;
    let radoviEndRow = null;

    if (obracun.length === 0) {
      const r = addRow5(["(nema stavki)", "", "", "", ""]);
      ws.mergeCells(`A${r.number}:E${r.number}`);
      r.font = { italic: true };
    } else {
      for (const st of obracun) {
        const rowNum = rowRadovi(st);
        if (radoviStartRow === null) radoviStartRow = rowNum;
        radoviEndRow = rowNum;
      }
    }

    // Total radova (formula SUM)
    labelValueAE("Ukupna cena radova", (cellE, rowNumber) => {
		  cellE.font = { bold: true };
		  cellE.numFmt = "#,##0.00";

		  if (radoviStartRow && radoviEndRow) {
		    cellE.value = { formula: `SUM(E${radoviStartRow}:E${radoviEndRow})` };
		  } else {
		    cellE.value = 0;
		  }

		  totalRadoviRows.push(rowNumber); // now works
		});

    // ---------------- MATERIJAL ----------------
    fullWidth("Zaduzen materijal", {
		  fill: { type: "pattern", pattern: "solid", fgColor: { argb: "FFF7F7F7" } },
		});

    const reversi = Array.isArray(nalog?.reversi) ? nalog.reversi : [];
const reversSubtotalCells = []; // row numbers (NOT E values) where revers subtotal is placed


    if (reversi.length === 0) {
      const r = addRow5(["(nema reversa)", "", "", "", ""]);
		  ws.mergeCells(`A${r.number}:E${r.number}`);
		  r.font = { italic: true };
    } else {
      for (const rev of reversi) {
        fullWidth(`Broj reversa: ${s(rev?.broj)}`, { font: { bold: true } });

        headerMaterijal();

        const zaduzenje = Array.isArray(rev?.zaduzenje) ? rev.zaduzenje : [];
        let matStartRow = null;
        let matEndRow = null;

        if (zaduzenje.length === 0) {
          const r = addRow5(["(nema zaduženja)", "", "", "", ""]);
          ws.mergeCells(`A${r.number}:E${r.number}`);
          r.font = { italic: true };
        } else {
          for (const z of zaduzenje) {
		        const rowNum = rowMaterijal(z);     // E has formula, good
		        if (matStartRow === null) matStartRow = rowNum;
		        matEndRow = rowNum;
		      }
        }
        labelValueAE("Ukupna cena materijala (revers)", (cellE, rowNumber) => {
		      cellE.font = { bold: true };
		      cellE.numFmt = "#,##0.00";

		      if (matStartRow && matEndRow) {
		        cellE.value = { formula: `SUM(E${matStartRow}:E${matEndRow})` };
		      } else {
		        cellE.value = 0;
		      }

		      reversSubtotalCells.push(rowNumber); // remember where this subtotal is
		      totalMaterijalRows.push(rowNumber); // ✅ THIS feeds the grand total
		    });
        // (sub is not used further; rowNumber captured above)

      }

      // Optional: nalog-level material total (sum of revers subtotals)
      /*labelValueAE("Ukupna cena materijala (nalog)", (cellE) => {
        cellE.font = { bold: true };
        cellE.numFmt = "#,##0.00";
        if (reversSubtotalCells.length > 0) {
          const parts = reversSubtotalCells.map((r) => `E${r}`).join(",");
          cellE.value = { formula: `SUM(${parts})` };
        } else {
          cellE.value = 0;
        }
      });*/
    ...
    ws.addRow(["", "", "", "", ""]); // spacer between nalogs
    const nalogEndRow = ws.lastRow.number;

	  // 🔥 APPLY BORDER AROUND THE WHOLE NALOG
	  borderNalog(ws, nalogStartRow, nalogEndRow);
  }
  ws.addRow(["", "", "", "", ""]); // spacer

		const gtTitle = ws.addRow(["UKUPNO – ZBIRNI IZVEŠTAJ", "", "", "", ""]);
		ws.mergeCells(`A${gtTitle.number}:E${gtTitle.number}`);
		gtTitle.font = { bold: true, size: 13 };
		gtTitle.alignment = { horizontal: "center", vertical: "middle" };

		ws.addRow(["", "", "", "", ""]); // spacer

		// ---- TOTAL RADOVI (all nalogs)
		const gtRadovi = ws.addRow(["Ukupno – svi nalozi (radovi)", "", "", "", ""]);
		ws.mergeCells(`A${gtRadovi.number}:D${gtRadovi.number}`);
		gtRadovi.getCell(1).font = { bold: true };

		gtRadovi.getCell(5).value = {
		  formula: totalRadoviRows.length
		    ? `SUM(${totalRadoviRows.map(r => `E${r}`).join(",")})`
		    : 0
		};
		gtRadovi.getCell(5).numFmt = "#,##0.00";
		gtRadovi.getCell(5).font = { bold: true };

		// ---- TOTAL MATERIJAL (all nalogs)
		const gtMat = ws.addRow(["Ukupno – sav materijal", "", "", "", ""]);
		ws.mergeCells(`A${gtMat.number}:D${gtMat.number}`);
		gtMat.getCell(1).font = { bold: true };

		gtMat.getCell(5).value = {
		  formula: totalMaterijalRows.length
		    ? `SUM(${totalMaterijalRows.map(r => `E${r}`).join(",")})`
		    : 0
		};
		gtMat.getCell(5).numFmt = "#,##0.00";
		gtMat.getCell(5).font = { bold: true };

		// Optional: thick border around grand totals
		borderNalog(ws, gtTitle.number, gtMat.number);
...
server.get('/kontrola/radnici/:datumYYYYMMDD',async (req,res)=>{
	if(!req.session.user){
		return res.redirect("/login?url="+encodeURIComponent(req.url));
	}
	try{
		var today = getDateAsStringForDisplay(new Date(req.params.datumYYYYMMDD));
		var majstori = await majstoriDB.find({uniqueId:{$nin:podizvodjaci}}).toArray();
		var izvestaji = await dnevniIzvestajiDB.find({date:{$regex:today.split(".")[2]+"-"+today.split(".")[1]}}).toArray();
		//var izvestaji = await dnevniIzvestajiDB.find({date: { $gt: "2025-10-14" }}).toArray();
		var brojeviNaloga = [];
...
server.get('/jucerasnjiRasporedRadova', async (req,res)=>{
	if(req.session.user){
		if(Number(req.session.user.role)==20 || Number(req.session.user.role)==25){
			try{
				var today = new Date();
...
server.get('/sutrasnjiRasporedRadova', async (req,res)=>{
	if(req.session.user){
		if(Number(req.session.user.role)==20 || Number(req.session.user.role)==25){
			try{
				var today = new Date();
...
server.get('/magacin/stavke',async (req,res)=>{
	if(req.session.user){
		if(Number(req.session.user.role)==10){
			try{
				var proizvodi = await proizvodiDB.find({}).toArray();
				res.render("administracija/magacinProizvodi",{
...
server.post('/izmenaProizvodaAdmin', async (req, res)=> {
	if(req.session.user){
		if(Number(req.session.user.role)==10){
			try{
				var json = JSON.parse(req.body.json);
				if(json.uniqueId){
					...
					var response = await proizvodiDB.updateOne({uniqueId:json.uniqueId},setObj)
					...
					json.stanjeistok = 0;
					var response = await proizvodiDB.insertOne(json);
				}
				res.redirect("/magacin/stavke?id="+json.uniqueId);
			}catch(err){
				logError(err);
				res.render("message",{
					pageTitle: "Програмска грешка",
...
server.get('/magacin/alati',async (req,res)=>{
	if(req.session.user){
		if(Number(req.session.user.role)==10){
			try{
				var alati = await alatDB.find({}).toArray();
				res.render("administracija/magacinAlati",{
...
server.post('/izmenaAlataAdmin', async (req, res)=> {
	if(req.session.user){
		if(Number(req.session.user.role)==10){
			try{
				var json = JSON.parse(req.body.json);
				if(json.uniqueId){
					var setObj	=	{ $set: {
								...
								regionName: json.region == "I" ? "istok" : "zapad",
								vrsta: json.vrsta,
								podvrsta: json.podvrsta
							}
					};
					var response = await alatDB.updateOne({uniqueId:json.uniqueId},setObj)
				}else{
					json.uniqueId = generateId(5)+"--"+new Date().getTime();
					var broj = await alatDB.countDocuments();
					broj = broj+1;
					json.redniBroj = broj.toString().padStart(4,"0");
					json.regionName = json.region == "I" ? "istok" : "zapad";
					var response = await alatDB.insertOne(json);
					
				}
				res.redirect("/magacin/alati?id="+json.uniqueId);
			}catch(err){
				logError(err);
				res.render("message",{
					pageTitle: "Програмска грешка",
...
server.get('/magacin/zaduzivanjeAlata',async (req,res)=>{
	if(req.session.user){
		if(Number(req.session.user.role)==10){
			try{
				var alati = await alatDB.find({}).toArray();
				var majstori = await majstoriDB.find({uniqueId:{$nin:podizvodjaci}}).toArray();
				var zaduzenje = await alatZaduzenjaDB.find({}).toArray();
				res.render("administracija/zaduzivanjeAlata",{
...
server.post('/zaduzenjeAlata', async (req, res)=> {
	if(req.session.user){
		if(Number(req.session.user.role)==10){
			try{
				var json = JSON.parse(req.body.json);
				...
				json.datetime = new Date().getTime();
				json.uniqueId = generateId(5)+"--"+new Date().getTime();
				var response = await alatZaduzenjaDB.insertOne(json);
				res.redirect("/magacin/zaduzenjaAlata?id="+json.uniqueId);
			}catch(err){
				logError(err);
				res.render("message",{
					pageTitle: "Програмска грешка",
...
server.get('/magacin/zaduzenjaAlata',async (req,res)=>{
	if(req.session.user){
		if(Number(req.session.user.role)==10){
			try{
				var alati = await alatDB.find({}).toArray();
...
server.get("/nedeljniIzvestaj/:start/:end/:opseg/:podizvodjaci", checkBearerToken, async (req, res) => {
    try{
    	function getPreviousWeekDates(date = new Date()) {
			  const d = new Date(date);
			...
			var previousDates = getDatesBetween(req.params.start, req.params.end);
			var previousDates2 = getDatesBetween(req.params.start, req.params.end);
	   	var query = getDateRangeQuery(req.params.start, req.params.end);
			...
			//
			var filterJson = {};
			if(req.params.podizvodjaci=="Da"){
				filterJson = {majstor:{$in:podizvodjaci},statusNaloga:{$in:["Fakturisan","Spreman za fakturisanje","Nalog u Stambenom"]},"prijemnica.datum.datum":{$in:previousDates}}
			}else{
				filterJson = {statusNaloga:{$in:["Fakturisan","Spreman za fakturisanje","Nalog u Stambenom"]},"prijemnica.datum.datum":{$in:previousDates}}
			}
	   	var nalozi = await naloziDB.find(filterJson).toArray();
	   	var nalogIds = [];
	   	for(var i=0;i<nalozi.length;i++){
	   		nalogIds.push(nalozi[i].broj)
	   	}
	   	...
	   	var json = {};
			...
			json.zamene = 0;
			json.zameneBroj = 0;
			json.zamene0_10Broj = 0;
			json.zamene0_10 = 0;
			json.zamene10_30Broj = 0;
			json.zamene10_30 = 0;
			json.zamene30_60Broj = 0;
			json.zamene30_60 = 0;
			json.zamene60_100Broj =0;
			json.zamene60_100=0;
			json.zamene100_150Broj = 0;
			json.zamene100_150 = 0;
			json.zamene150_250Broj = 0;
			json.zamene150_250 = 0;
			json.zamene250Broj = 0;
			json.zamene250 = 0;
			json.ukupno = 0;
			json.ukupnoBroj = 0;
			json.trecaLica = 0;
			...
			//function handleData(opseg,json,regionName){
			var regionName = req.params.opseg;
			var opseg = req.params.opseg;
			if(req.params.opseg=="BEOGRAD"){
				opseg = radneJedinice;
			}else if(req.params.opseg=="ZAPAD"){
				opseg = zapad;
			}else if(req.params.opseg=="ISTOK"){
				opseg = istok;
			}
			...
			for(var i=0;i<nalozi.length;i++){
				...
				if(opseg.indexOf(nalog.radnaJedinica)>=0){
					...
					for(var j=0;j<nalozi[i].obracun.length;j++){
						if(sifraZamene.indexOf(nalozi[i].obracun[j].code)>=0 && parseFloat(nalozi[i].ukupanIznos)<10000){
							json.zamene0_10Broj++;
							json.zamene0_10 = json.zamene0_10 + parseFloat(nalozi[i].ukupanIznos);
							break;
						}else if(sifraZamene.indexOf(nalozi[i].obracun[j].code)>=0 && parseFloat(nalozi[i].ukupanIznos)>=10000 && parseFloat(nalozi[i].ukupanIznos)<=30000){
							json.zamene10_30Broj++;
							json.zamene10_30 = json.zamene10_30 + parseFloat(nalozi[i].ukupanIznos);
							break;
						}else if(sifraZamene.indexOf(nalozi[i].obracun[j].code)>=0 && parseFloat(nalozi[i].ukupanIznos)>=30000 && parseFloat(nalozi[i].ukupanIznos)<=60000){
							json.zamene30_60Broj++;
							json.zamene30_60 = json.zamene30_60 + parseFloat(nalozi[i].ukupanIznos);
							break;
						}else if(sifraZamene.indexOf(nalozi[i].obracun[j].code)>=0 && parseFloat(nalozi[i].ukupanIznos)>60000 && parseFloat(nalozi[i].ukupanIznos)<100000){
							json.zamene60_100Broj++;
							json.zamene60_100 = json.zamene60_100 + parseFloat(nalozi[i].ukupanIznos);
							break;
						}else if(sifraZamene.indexOf(nalozi[i].obracun[j].code)>=0 && parseFloat(nalozi[i].ukupanIznos)>=100000 && parseFloat(nalozi[i].ukupanIznos)<150000){
							json.zamene100_150Broj++;
							json.zamene100_150 = json.zamene100_150 + parseFloat(nalozi[i].ukupanIznos);
							break;
						}else if(sifraZamene.indexOf(nalozi[i].obracun[j].code)>=0 && parseFloat(nalozi[i].ukupanIznos)>=150000 && parseFloat(nalozi[i].ukupanIznos)<250000){
							json.zamene150_250Broj++;
							json.zamene150_250 = json.zamene150_250 + parseFloat(nalozi[i].ukupanIznos);
							break;
						}else if(sifraZamene.indexOf(nalozi[i].obracun[j].code)>=0 && parseFloat(nalozi[i].ukupanIznos)>=250000){
							json.zamene250Broj++;
							json.zamene250 = json.zamene250 + parseFloat(nalozi[i].ukupanIznos);
							//console.log("ISTOK VELIKI ("+nalozi[i].vrstaRada+"): "+brojSaRazmacima(parseFloat(nalozi[i].ukupanIznos)))
							break;
						}
					}
...
io.on('connection', function(socket){
	...
	socket.on('adresaPoBroju', async function(broj){
		try{
			var nalozi = await naloziDB.find({broj:broj.toString()}).toArray();
			if(nalozi.length>0){
				socket.emit('adresaPoBrojuOdgovor',nalozi[0].adresa)
			}else{
				var nalozi = await naloziTrecihLicaDB.find({broj:broj}).toArray();
				if(nalozi.length>0){
					socket.emit('adresaPoBrojuOdgovor',nalozi[0].adresa)
				}
			}
